...
interface AlignmentTestFormProps {
  locale: Locale;
}

export default function AlignmentTestForm({ locale }: AlignmentTestFormProps) {
  const t = getTranslations(locale).alignmentTest;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/alignment-test", {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-md mx-auto">
      ...
      <p className="text-gray-500 text-xs text-center leading-relaxed">{t.privacyLine}</p>
    </form>
  );
}
